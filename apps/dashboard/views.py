...
class DashboardView(CacheMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        last_month_portfolio = Credit.objects.filter(
            status="active", created__lte=last_month
        ).aggregate(
            total=Coalesce(
                Sum("outstanding_balance"),
                Value(0),
        ...
        if last_month_portfolio > 0:
            portfolio_change = (
                (total_portfolio - last_month_portfolio) / last_month_portfolio
            ) * 100
        ...
        overdue_amount = overdue_installments.aggregate(
            total=Coalesce(
                Sum("installment_amount"), Value(0), output_field=DecimalField()
            )
        )["total"] or Decimal("0")
        ...
        if last_month_overdue > 0:
            overdue_change = (
                (overdue_amount - last_month_overdue) / last_month_overdue
            ) * 100
        ...
        monthly_goal = total_portfolio * Decimal(
            "0.15"
        )  # 15% de la cartera como meta
        goal_percentage = 0
        if monthly_goal > 0:
        ...
        credits_approved = credits_recent.filter(
        ...
        portfolio_by_product = (
            Credit.objects.filter(status="active")
        ...
        magic_links_used = MagicPaymentLink.objects.filter(
            status="used"
        ).count()
        magic_links_active = MagicPaymentLink.objects.filter(
            status="active"
        ).count()
        magic_links_expired = MagicPaymentLink.objects.filter(
            status="expired"
        ).count()
        ...
        top_debtors_qs = (
            Partner.objects.filter(credits__status="active")
            .annotate(
        ...
        top_debtors = [
            {
                ...
                "risk_level": "high"
                if partner.total_overdue > 30000
                else "medium"
                if partner.total_overdue > 10000
                else "low",
            }
        ...
        mora_ranges = {
            "1-30": overdue_installments.filter(
            ...
            "90+": overdue_installments.filter(
                due_date__lt=today - timedelta(days=90)
            ).count(),
        }

        # Morosidad por región/ciudad
        morosidad_por_region = (
            Partner.objects.filter(credits__status="active")
            .values("city")
            .annotate(
                total_overdue=Coalesce(
                    Sum(
                        "credits__installments__installment_amount",
                        filter=Q(
                            credits__installments__status="pending",
        ...
        try:
            campaigns_stats = Campaign.objects.aggregate(
                ...
                total_target=Coalesce(
                    Sum("target_amount"), Value(0), output_field=DecimalField()
                ),
            )
        ...
        social_security_pending = SocialSecurity.objects.filter(
            status="pending"
        ).aggregate(
            count=Count("id"),
            total=Coalesce(
